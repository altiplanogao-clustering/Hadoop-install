...
class EnvFile:
    ...
    def _as_user_pam_environment(owner_home, owner, group):
        return EnvFile(filename=os.path.join(owner_home, ".pam_environment"),
                        owner=owner,
                        group=group,
                        touch=True)
    ...
    def _as_user_profile(owner_home, owner, group):
        return EnvFile(filename=os.path.join(owner_home, ".profile"),
                        owner=owner,
                        group=group,
                        touch=True,
    ...
    def _as_sys_etc_environmenet():
        return EnvFile(filename="/etc/environment", touch=False)

    # @staticmethod
    # def sys_env_var_etc_profile():
    #     return ActionModule.env_file_template(filename="/etc/profile",
    #                                           touch=False)
    @staticmethod
    def _as_sys_etc_profile_d(filename):
        ...
        return EnvFile(filename=os.path.join("/etc/profile.d/", filename),
                        touch=True,
                        export_leading='export ')
    ...
    def as_user_env_files(fd, owner_home, owner, group):
        if fd == 'pam_env':
            return EnvFile._as_user_pam_environment(owner_home=owner_home, owner=owner, group=group)
        elif fd == 'profile':
            return EnvFile._as_user_profile(owner_home=owner_home, owner=owner, group=group)
        return None
    ...
    def as_sys_env_files(fd, filename):
        if fd == 'etc_env':
            return EnvFile._as_sys_etc_environmenet()
        elif fd == 'etc_profd':
            return EnvFile._as_sys_etc_profile_d(filename=filename)
        return None


class EnvFileDict:
    def __init__(self):
        dict_file = os.path.join(os.path.dirname(__file__), 'env_file_dict.yml')
    ...
    @staticmethod
    def _get_next_env_files(dict, def_val, next_path):
        def_key='default'
        next_dict = dict.get(next_path, None)
        if next_dict != None:
            next_dict_default = next_dict.get(def_key, None)
            node_def = EnvFileDict._merge(def_val, next_dict_default)
    ...
    def _get_env_files(dict, def_val, next_paths =[]):
        if len(next_paths) > 0:
            ...
            next_lvl = EnvFileDict._get_next_env_files(dict=dict, def_val=def_val, next_path=first)
            if next_lvl[0]:
                return EnvFileDict._get_env_files(dict=next_lvl[2], def_val=next_lvl[1], next_paths=next_paths)
            else:
                return next_lvl[1]
        else:
            return EnvFileDict._merge(def_val, dict)
    ...
    def envfiles(self, distribution):
        ...
        return EnvFileDict._get_env_files(self.__dict, def_val=def_node, next_paths=path)

    @staticmethod
    def load_envfile_dict():
        return EnvFileDict()

class Distribution:
    envfile_dict = EnvFileDict.load_envfile_dict()
    ...
    def user_env_files(self, owner_home='', owner='', group=''):
        ...
        for f in efd.get('user', []):
            env_files.append(EnvFile.as_user_env_files(f, owner_home=owner_home, owner=owner, group=group))
        return env_files
    ...
    def sys_env_files(self, profile_filename):
        ...
        for f in efd.get('system', []):
            env_files.append(EnvFile.as_sys_env_files(f, filename=profile_filename))
        return env_files
...
class ActionModule(ActionBase):
    ...
    def determine_user_env_var_file(task_vars, owner_home='', owner='', group=''):
        ...
        if (distribution['name'] == "Ubuntu"):
            user_files.append(EnvFile.as_user_pam_environment(owner_home=owner_home, owner=owner, group=group))
        return user_files
    ...
    def determine_sys_env_var_file(task_vars, profile_filename):
        ...
        user_files=[]
        if (distribution['name'] == "Ubuntu"):
            user_files.append(EnvFile.as_sys_etc_environmenet())
            # user_files.append(ActionModule.sys_env_var_etc_profile_d(profile_filename))
        return user_files

    def _write_env_var_file(self, task_vars, orig_args, present=True, env_files=[], tmp=None):
        ...
        for ef in env_files:
            ...
            step_result = self._execute_module(module_name="blockinfile",
                                          ...
                                          task_vars=task_vars,
                                          tmp=tmp)
            step_result['EnvFile'] = ef.filename
        ...
        if len(step_results) == 0:
            result={
                'failed': True,
                'changed': False,
                'msg': "No environment file located."
            }
            return result
        else:
            result = {}
            files_string=', '.join(x.get('EnvFile') for x in step_results)
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        profile_filename = orig_args.get('profile_filename', '')
